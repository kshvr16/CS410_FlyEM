...
#                     plt.show()
    ...
    def segment_cpu(src_dir, dest_dir):
        ...
        print("Copying all the images from source to real image directory")
        Utils.a2b_copy(src_dir, real)
        print("Copying all the images from real to original image directory")
        Utils.a2b_random(real, directories["originals"], 1)
        Utils.count_files(directories["originals"])  # count of the image files copied to the originals directory
        print("Copying the images from original to normalized and cropped")
        Utils.a2b_copy(directories["originals"], directories["normalized"])
        Utils.a2b_copy(directories["originals"], directories["cropped"])

        # Normalization to the original images
        print("Normalized the images of normalized dir and cropped dir")
        Utils.norm(directories["normalized"])
        Utils.norm(directories["cropped"])

        # Cropping of the normalized images
        print("Crop the images from cropped dir")
        Utils.crop(directories["cropped"])

        # Segmentation process
        print("Start segmentation process")
